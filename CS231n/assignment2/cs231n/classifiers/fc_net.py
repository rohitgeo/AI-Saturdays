...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params['W'+str(i+1)] = weight_scale * np.random.randn(input_dim, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        scores, cache = affine_forward(out, self.params['W'+str(i+1)], self.params['b'+str(i+1)])
        caches.append(cache)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        lyr =  self.num_layers
        dx, grads['W{}'.format(lyr)] , grads['b{}'.format(lyr)] = affine_backward(dx, caches.pop())
        
        for lyr in range(self.num_layers-1, 0, -1):
            if self.use_dropout:
            ...
            if self.use_batchnorm:
                ...
                grads['gamma{}'.format(lyr)], grads['beta{}'.format(lyr)] = dgamma, dbeta

            dx, grads['W{}'.format(lyr)] , grads['b{}'.format(lyr)] = affine_backward(dx, caches.pop())
        
        for lyr in range(1, self.num_layers+1):
            grads['W{}'.format(lyr)] += self.reg * self.params['W{}'.format(lyr)]
